...
Currently, the message processing is divided into two distinct steps.
...
\end{center}
Next, as shown in Figure 1.2, a batch process is scheduled to run twice a day to process and send the messages that have been previously stored to the DispatcherAPI, which then notifies the clients.
\begin{center}
    \centering
...
This system will connect directly to RabbitMQ as a data source, will process events as they arrive, then send them to DispatcherAPI. The latter will in turn be responsible for distributing them to customers, thus providing immediate responsiveness and improving the customer experience by guaranteeing instant receipt of notifications.
...
Such methodology contains a set of principles that need to be respected and fol
...
\begin{itemize}
    ...
    If necessary, sshe also coaches and mentors the
    team.
...
\section*{Conclusion}
In the first chapter, we started with the presentation of the host organisation.
Then we presented the software requirements adopted to assure the finest software quality for our project will be examined in the next chapter.