...
The exponential growth of mobile data usage has made real-time monitoring of customer consumption a critical concern for telecommunications operators.
Users increasingly expect to be notified instantly when they approach or exceed their data consumption threshold, so they can react before experiencing any service disruption.

Meeting this expectation raises a central question: how can we deliver accurate consumption notifications to customers with minimal latency, while keeping the system scalable and reliable as the volume of events grows?
Answering this question requires a close look at the current notification process, its limitations, and the improvements that a real-time approach can bring, which is the focus of the following sections.

\section{State of the art}

Our project, called \("\)Levee Fair Use\("\) or LFU in short, aims to set up a real-time data processing system for push notifications that let users know when they have reached their data consumption threshold.
In the era of massive data proliferation, processing such streams in real time constitutes a major challenge: reacting instantly to events requires a robust and efficient system able to meet customer requirements while guaranteeing real-time responsiveness.
Before designing this system, it is essential to examine how the notification process currently works and to identify its limitations.
This section therefore studies the existing method, points out its shortcomings, and introduces the solution we propose to overcome them.
...
\end{center}
\subsection{Critic of the existing method}
...
This section is dedicated to the explanation of the agile project management methodology utilized to construct the solution which is the Scrum methodology.

\subsection{Scrum definition}
Scrum methodology is one of the most widely used agile development methodologies out there.
