...
\section{Complete Pipeline Topology}

With all operators in place, the full Flink~\cite{FLINK} topology is constructed as follows:
...
The \texttt{LFUUpdateSubscriptionSink} class updates the \texttt{LfuMainRecord}'s state in Valkey~\cite{valkey} with the timestamp of the last processed event. This enables the next processing cycle to compute the time interval since the last notification.
