...
Our end result for this spring is being able to package the LFU job as a production-ready Docker image, configure high-availability deployment, and establish Flink checkpointing for fault tolerance.
...
FROM *(repository url)*/flink_base:1.20.1_scala_2.12_FIX
...
Figure~\ref{fig:helm-deployment} illustrates the different Helm deployment processes of our Flink job.

\begin{center}
    \centering
    \includegraphics[width=1\textwidth]{img/Helm deployment}
    \captionof{figure}{Helm deployment process}
    \label{fig:helm-deployment}
\end{center}
...
\begin{itemize}
    ...
    \item[-] \textbf{ConfigMaps:} contain the configuration of our Flink job and are injected as YAML files.
\end{itemize}

Figure~\ref{fig:k8s-resources} shows the various Kubernetes resources deployed for our Flink job.

\begin{center}
    \centering
    \includegraphics[width=1\textwidth]{img/Kubernetes objects}
    \captionof{figure}{Kubernetes resources deployed for the Flink job}
    \label{fig:k8s-resources}
\end{center}
...
\begin{itemize}
    ...
    \item \textbf{TaskManagers} with 2 task slots each (configurable via \texttt{TASK\_MANAGER\_} \\ \texttt{NUMBER\_OF\_TASK\_SLOTS}).
    \item \textbf{Checkpoints} stored on the shared filesystem (\texttt{/opt/flink/checkpoints}).
    \item \textbf{HA metadata} stored in the local filesystem (\texttt{/opt/flink/flink-ha}).
\end{itemize}


\section{Checkpoints and Fault Tolerance}

Checkpoints make Flink's state fault tolerant since it allows the recovery of state and the corresponding stream positions to be recovered after being saved in our S3~\cite{s3} storage.
Thereby allowing us to give our application the same semantics as a failure-free execution.


\begin{lstlisting}[language=Java, caption={Checkpoint configuration (LFUJob.java)}, breaklines=true]
if (CHECKPOINT_ENABLED) {
    env.enableCheckpointing(
        Long.parseLong(CHECKPOINT_INTERVAL_IN_MS),
        CHECKPOINTING_MODE);
...
\begin{itemize}
    \item \textbf{Mode}: We use the mode \texttt{AT\_LEAST\_ONCE} (required for correlation ID).
...
Figure~\ref{fig:s3-checkpoints} shows the Scality graphic interface, a tool that provides access to our S3 buckets in order to visualize and manage the data stored in them. It allows us to observe the ten latest checkpoints saved by our Flink job.

\begin{center}
    \centering
    \includegraphics[width=1\textwidth]{img/s3}
    \captionof{figure}{Scality S3 browser: checkpoints saved by the Flink job}
    \label{fig:s3-checkpoints}
\end{center}
...
The CI/CD pipeline triggers each time a commit is made to the working branch. It is divided into three stages:

\begin{center}
    \centering
    \includegraphics[width=1\textwidth]{img/CICD pipeline}
    \captionof{figure}{CI/CD workflow}
...
\begin{enumerate}
    ...
    \begin{center}
        \centering
        \includegraphics[width=0.65\textwidth,height=9.5cm]{img/Build job}
        \captionof{figure}{Build stage result}
        \label{fig:job-build}
    \end{center}
    ...
    \end{itemize}
    \begin{center}
        \includegraphics[width=13.5cm, height=7cm]{img/fortify job}
        \captionof{figure}{Fortify stage result}
        \label{fig:Fortify-build}
    \end{center}
    \begin{itemize}
    ...
    \begin{center}
        \centering
        \includegraphics[width=13.5cm, height=7cm]{img/sonar job}
        \captionof{figure}{Sonar stage result}
        \label{fig:Sonar-build}
    \end{center}
...
\begin{center}
    \centering
    \includegraphics[width=1\textwidth]{img/docker job}
    \captionof{figure}{Docker stage result}
...
\texttt{USE\_CORRELATION\_ ID} & false & Enable correlation ID tracking \\
\midrule
\multicolumn{3}{|l|}{\textit{Flink}} \\
\hline
\texttt{GLOBAL\_PARALLELISM} & 2 & Global job parallelism \\
\texttt{CHECKPOINT\_ENABLED} & false & Enable checkpointing \\
\texttt{CHECKPOINT\_ INTERVAL\_IN\_MS} & 1000 & Interval between checkpoints (ms) \\
\texttt{CHECKPOINT\_MODE} & EXACTLY\_ ONCE & Checkpointing mode \\
\texttt{USE\_SNAPSHOT\_ COMPRESSION} & false & Snapshot compression \\
\texttt{LATENCY\_INTERVAL} & --- & Latency tracking interval \\
\midrule
\multicolumn{3}{|l|}{\textit{Valkey}} \\
\hline
\texttt{VALKEY\_HOST\_HTTP} & --- & Valkey API URL \\
\texttt{VALKEY\_BASE\_PATH} & --- & API base path \\
\midrule
...
\hline
\texttt{AI\_ANOMALY\_ THRESHOLD} & 0.85 & Anomaly detection threshold \\
\texttt{AI\_MODEL\_PATH} & /models/fairuse\_ prediction.onnx & ONNX model path \\
\bottomrule
    \end{tabularx}
    \captionof{table}{Main environment variables}
    \label{tab:env-vars}
\end{center}
