...
Figure~\ref{fig:flink-topology} represents the Flink job topology, illustrating how the different job components interact with each other to process data. More specifically, it shows the data path through the different operations of our job, from the initial source to the final destination.
...
Figure~\ref{fig:flink-operators} offers a detailed overview of the different deployed operators, the data processed and exchanged between them, as well as their deployment statuses. We notice that each operator is deployed twice due to the job parallelism, which is set to 2. This allows us to understand the entire data flow and identify the steps where transit issues may have been encountered.

\begin{center}
    \centering
    %\includegraphics[width=1\textwidth]{img/flink_operators}
    \captionof{figure}{Operators visualization}
    \label{fig:flink-operators}
\end{center}
